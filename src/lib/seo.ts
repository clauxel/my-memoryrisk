...
const liveOrigin = 'https://memoryrisk.space'
...
const canonicalLinkId = 'memoryrisk-canonical-link'
const structuredDataScriptId = 'memoryrisk-structured-data'
...
function resolveCanonicalOrigin(origin: string) {
  try {
    ...
    if (url.hostname.endsWith('.pages.dev') || url.hostname.endsWith('.workers.dev')) return url.origin
  } catch {}
...
export function buildSeoDocument(args: {
  ...
  if (routeView === 'pricing') {
    const title = 'MemoryRisk Pricing | Team Annual Shortage Risk Platform'
    const description =
      'Compare MemoryRisk Watchlist, Team, and Channel plans. Team annual is selected by default and annual billing is 50% cheaper than monthly.'
    return {
      title,
      description,
      canonicalUrl,
  ...
  if (routeView === 'privacy') {
    ...
    const description = 'How MemoryRisk handles analytics, checkout metadata, BOM uploads, reports, and public site interactions.'
    return {
      title,
      description,
      canonicalUrl,
  ...
  if (routeView === 'terms') {
    ...
    const description = 'Terms for using MemoryRisk, hosted checkout, BOM uploads, rule-based procurement scoring, and reports.'
    return {
      title,
      description,
      canonicalUrl,
  ...
  return {
    ...
    description: 'The requested MemoryRisk page was not found.',
    canonicalUrl,
    robots: 'noindex,nofollow',
